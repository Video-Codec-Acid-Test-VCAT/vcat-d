...
/* package */ final class AtomParsers {
  ...
  private static StsdData parseStsd(
    ...
    for (int i = 0; i < numberOfEntries; i++) {
      ...
      if (childAtomType == Atom.TYPE_avc1
              ...
              || VcatDecoderManager.getInstance().getNonStandardDecoders().containsKey(childAtomType)) {
  ...
  private static boolean isNonStandardVideo(int sampleEntryFourCC, int videoCfgFourCC){
    if(VcatDecoderManager.getInstance().getNonStandardDecoders().containsKey(sampleEntryFourCC)){
      NonStdDecoderStsdParser parser =VcatDecoderManager.getInstance().getNonStandardDecoders().get(sampleEntryFourCC);
  ...
  private static void parseVideoSampleEntry(
    ...
    while (childPosition - position < size) {
      ...
      } else if(isNonStandardVideo(atomType, childAtomType)){
        ExtractorUtil.checkContainerInput(mimeType == null, null);
        parent.setPosition(childStartPosition);

        byte[] atomBuffer = new byte [childAtomSize];
        parent.setPosition(childStartPosition);
        parent.readBytes(atomBuffer, 0, childAtomSize);

        NonStdDecoderStsdParser parser =VcatDecoderManager.getInstance().getNonStandardDecoders().get(atomType);
        mimeType = parser.mimeType();

        VideoConfiguration cfg = parser.parseStsd(atomBuffer);
        if(cfg != null) {
          initializationData = cfg.initializationData;
        }

        out.nalUnitLengthFieldLength = cfg.nalUnitLengthFieldLength;
        if (!pixelWidthHeightRatioFromPasp) {
          pixelWidthHeightRatio = cfg.pixelWidthHeightRatio;
        }
        codecs = cfg.codecs;
        colorSpace = cfg.colorSpace;
        colorRange = cfg.colorRange;
        colorTransfer = cfg.colorTransfer;
